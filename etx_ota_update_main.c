...
int send_ota_start(int comport)
{
  ...
  ota_start->data_len_1     = 0;
  ota_start->data_len_2     = 1;
...
uint16_t send_ota_end(int comport)
{
  ...
  ota_end->data_len_1     = 0;
  ota_end->data_len_2     = 1;
...
int send_ota_header(int comport, int app_size)
{
  uint16_t len;
  ...
  ota_header->data_len_1     = 0;
  ota_header->data_len_2     = app_size;
...
int send_ota_data(int comport, uint8_t *data, uint16_t data_len)
{
  ...
  ota_data->data_len_1     = data_len>>8;
  ota_data->data_len_2     = data_len;
  ota_data->eof          = ETX_OTA_EOF;
...
int main(int argc, char *argv[])
{
  ...
  do
  {
    if( argc <= 2 )
    {
      ...
      printf("Example: .\\etx_ota_app.exe 3 ..\\..\\Application\\Debug\\Application_version2.bin");
      ex = -1;
      break;
    }
    ...
    //Send OTA Header
    ex = send_ota_header( comport, app_size/ETX_OTA_DATA_MAX_SIZE );
    if( ex < 0 )
    {
